...
const MainUI = () => {
  ...
  const id = localStorage.getItem("seller-token");

  const getAllData = async () => {
    try {
      const res = await axios.get(`http://localhost:8000/get-seller-product/${id}`);
      setData(res?.data?.product?.products);
      setFilteredData(res?.data?.product?.products);
    } catch (error) {
          ...
          {filteredData.length === 0 ? (
            <div className="flex justify-center mt-10">
              <span className="text-4xl font-bold text-gray-500 py-12">
                <img src="https://img.freepik.com/free-vector/no-data-concept-illustration_114360-626.jpg?size=626&ext=jpg" alt="No data" />
              </span>
            </div>
          ) : (
                  ...
                  <div key={item._id} className='w-[47%] sm:w-[23%] md:w-[36%] lg:w-1/6'>
                    <div key={item.id} className="relative m-2  lg:m-5 border rounded-lg border-[#EB6440] flex flex-col w-full h-full max-w-xs overflow-hidden shadow-xl hover:shadow-gray-500 bg-white">
                      <div className="absolute top-0 right-0 flex gap-2 p-2">
                        <button onClick={() => navigate(`/edit-product/${item._id}`)} className="text-green-700 z-[99999]">
                          <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-pencil"><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z" /><path d="m15 5 4 4" /></svg>

                        </button>
                        <button onClick={() => handleDelete(item._id)} className="text-red-500 z-[99999] hover:text-red-700 ">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={4} stroke="currentColor" className="w-8 h-8 font-bold">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M20 9L9 20M9 9l11 11" />
                          </svg>
                        </button>
                      </div>
